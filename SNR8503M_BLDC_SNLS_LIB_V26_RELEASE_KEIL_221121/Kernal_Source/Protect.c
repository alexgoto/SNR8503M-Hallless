...
void Current_Limit(void)
{
	...
	{
		User_sys.BLDC_SensorlessCtr.peakBusCurrentIn = User_sys.BLDC_SensorlessCtr.peakBusCurrent;
	}
	...
	u32Vol_V_Final = u32Vol_ADC_Final*3.6/VOLTAGE_SHUNT_RATIO/32752;			//(LOW_VOLATAGE_THD_2 * VOLTAGE_SHUNT_RATIO/3.6*32752)  													
...
void OVER_current_protect(void)
{
   if(Motor_1M1st.BLDC.u8FlagMotorIsRunning == 1)		/*运行过程*/
	 {
...
void Vol_protect(void)
{
	...
	if(Motor_1M1st.BLDC.u8FlagMotorIsRunning == 1)		/*运行过程中欠压*/
	{
	...
	else		/*开机之前欠压*/
	{
		...
		{  
			if(Motor_1M1st.BLDC.u8FlagEnMotorRun == 1)			// Speed command enable motor startup
			{
...
{
	u8 i;
	if(Motor_1M1st.BLDC_CMP.u8FlagGetMotorElePeriod == 1)
	{	
		Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg = Motor_1M1st.BLDC_CMP.u32ElectricalPeriod/6;
		Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg_2 = Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg << 1;
		Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg_1_2 = Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg >> 1;
		Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg_1_4 = Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg_1_2 >> 1;
		Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg_3_2 = Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg_2 - Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg_1_2;
		
		if(Motor_1M1st.BLDC_CMP.u32closeloop_count > 5)
		{			
			#if EN_MOTOR_BLOCK_DETECT
				Motor_1M1st.BLDC_CMP.u16MotorShakeCnt = 0;
			
				Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[6] = Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[0];
				for(i=0; i<6; i++)
				{
					// 条件 1
//					if((Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[i] < Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg_1_2) || \
//						(Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[i] > Motor_1M1st.BLDC_CMP.u32MotorElePhaseAvg_2))
//					{
//						Motor_1M1st.BLDC_CMP.u16MotorShakeCnt++;
//					}
					
				//	// 条件 2
//					if((Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[i + 1] >> 1) > Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[i])
//					{
//						Motor_1M1st.BLDC_CMP.u16MotorShakeCnt++;
//					}
					
				//	// 条件 3
//					if( Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[i + 1] < (Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[i]) >> 1)
//					{
//						Motor_1M1st.BLDC_CMP.u16MotorShakeCnt++;
//					}					
	
				//	// 条件 4
//					if( (Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[i] > (MAX_SPEED_CNT - 20)) && \
//						Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[i + 1] > (MAX_SPEED_CNT - 20))
					if(Motor_1M1st.BLDC_CMP.u32MotorElePhaseCnt[i] > (MAX_SPEED_CNT - 20))
					{
						Motor_1M1st.BLDC_CMP.u16MotorShakeCnt++;
					}		
					
					if(Motor_1M1st.BLDC_CMP.u16MotorShakeCnt > 0)
					{
						Motor_1M1st.BLDC_CMP.u16MotorShakeDelayProtectCnt++;
					}
					else
					{
						if(Motor_1M1st.BLDC_CMP.u16MotorShakeDelayProtectCnt > 0)
						Motor_1M1st.BLDC_CMP.u16MotorShakeDelayProtectCnt--;
					}
				}

				if(Motor_1M1st.BLDC_CMP.u16MotorShakeDelayProtectCnt > MOTOR_BLOCK_DETECT_CNT)
				{
