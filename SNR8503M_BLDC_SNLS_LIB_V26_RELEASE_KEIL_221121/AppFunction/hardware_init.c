...
void GPIO_init(void)
{
    ...
    /*P1.4 PG*/
    GPIO_InitStruct.GPIO_Mode = GPIO_Mode_IN;  /*GPIO输入模式*/
    GPIO_InitStruct.GPIO_Pin = GPIO_PIN_PG;     
    GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_UP;  /*GPIO上拉*/
    GPIO_Init(GPIO_PG, &GPIO_InitStruct);
    ...
    GPIO_Init(GPIO_LED, &GPIO_InitStruct);		
		...
		/*P1.8  P1.9 SWD  Pin9 NTC_AD*/
		#if EN_MOS_TEMP_DETECT
		Multiplex_SWD();
    GPIO_InitStruct.GPIO_Mode = GPIO_Mode_IN;  /*GPIO输入模式*/
    GPIO_InitStruct.GPIO_Pin = GPIO_Pin_9;     /*Pin9*/
		GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_NOPULL;  /*GPIO无上拉*/
...
void ADC_init(void)
{
    ...
    ADC_CHN0 = ADC_PEAK_CUR_CHANNEL  | (BEMF_CH_A << 4) | ( BEMF_CH_B << 8) | (BEMF_CH_C << 12);
		ADC_CHN1 = ADC_BUS_VOL_CHANNEL | (ADC_SPEED_CHANNEL << 4) | (ADC_MOS_TEMP_CHANNEL << 8);
...
void UTimer_init(void)
{
    ...
    TIM_InitStruct.CH0_FE_CAP_EN = DISABLE;     /* 关闭Timer 通道0下降沿捕获*/
    TIM_InitStruct.CH0_RE_CAP_EN = DISABLE;     /* Timer 通道0上升沿捕获事件使能*/
    TIM_InitStruct.SRC0 = TIM_SRC1_0;           /* 捕获模式通道1信号来源,Timer通道0*/
    TIM_InitStruct.CAP0_CLR_EN = DISABLE;       /* 当发生CAP0捕获事件时，清零Timer计数器*/
    TIM_InitStruct.TH = TIMER1_TH_VALUE;     		/* 设置计数器计数模值*/
    TIM_InitStruct.FLT = 0;                     /* 通道 0/1 信号滤波宽度选择，0-255 */
    TIM_InitStruct.CLK_DIV = TIM_Clk_Div1;      /* 设置Timer模块数据分频系数 */
...
void DAC_init(void)
{
    ...
    //SYS_AFE_DAC = SHORT_CURRENT_DAC;                     /* 硬件过流保护阈值 */
...
void CMP_init(void)
{
    ...
    SYS_AnalogModuleClockCmd(SYS_AnalogModule_CMP0,ENABLE); 	/* 比较器0开关 */
    ...
    CMP_InitStruct.CMP0_SELN = CMP0_SELN_HALL_MID;
    CMP_InitStruct.CMP0_SELP = CMP0_BEMF_CH_U; 		 /* CMP0_P CMP0_IP0母线 */
    CMP_InitStruct.CMP0_IN_EN = ENABLE;            /* 比较器信号输入使能 */
